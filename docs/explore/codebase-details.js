...
if (auto drawn = commands.draw(*gpu_mesh); !drawn) {
  ...
  describe("vng_resources_opengl", [
    section("Build a ready owner, not a half-initialized object", "render::mesh_renderer_builder(device) collects mesh, program and optional image providers. build returns a ready opengl::MeshRenderer or a diagnostic. The owner stores realized resources and recipes. Tickets describe draw-time values; the engine does not force every custom renderer to use this builder or implementation.", "include/vng/opengl/mesh_renderer.hpp"),
...
if (!bloom) return fail(bloom.error());`, true)
  ...
  describe("vng_rig_opengl", [
    section("Realize the binding once, update the pose often", "The skinned renderer uploads geometry, influences and shader resources from a neutral SkinBinding. A SkinnedDraw supplies pose and instance transform. Changing a pose updates the bone palette; it should not require rebuilding the mesh or moving rest positions on the CPU. Independent characters may share a binding while retaining different poses.", "include/vng/opengl/skinned_mesh_renderer.hpp"),
  ...
  describe("vng_ui_opengl", [
    section("Render a Screen without taking ownership of it", "The UI renderer consumes neutral drawing data for widget shapes and text. It owns its OpenGL realization and a text renderer, not the Screen's widget state. Input/layout can therefore be tested in vng_ui without a GL context, while pixel/blending behavior is tested in the integration.", "include/vng/opengl/ui_renderer.hpp"),
