...
TEST_CASE("Asteroid belt occludes the fleet and reveals it using the editable camera", "[asteroid][opengl]") {
    ...
    const auto render=[&](const project::State& value,f32 time,bool diagnostic=false) {
        const auto view=project::camera(*project::evaluate_camera(value,time),project::ViewMode::scene);
        return runtime->render(*device,{value,view,extent,time,diagnostic});
    };
    auto no_fleet=*state;
    ...
    for(const auto time:{0.F,10.F,18.F,26.F,38.F,52.F}) {
        auto full=render(*state,time); REQUIRE(full);
        auto empty=render(no_fleet,time); REQUIRE(empty);
        const auto pixels=changed(*full,*empty);
        auto unobstructed=render(clear_fleet,time); REQUIRE(unobstructed);
        auto background=render(clear_empty,time); REQUIRE(background);
    ...
    auto diagnostic=render(*state,0.F,true); REQUIRE(diagnostic);
