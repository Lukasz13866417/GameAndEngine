...
// Shared implementation of bounded, timeline-aware scalar/vng::Vec3 properties.
...
public:
    ...
    static_assert(std::same_as<Value, vng::Vec3> || std::same_as<Value, vng::f32>);
    std::array<char, 8> magic;
    std::string_view name;
    std::string_view label;
    vng::f32 limit;
    vng::f32 minimum{-limit};
    static constexpr std::size_t value_bytes = std::same_as<Value, vng::Vec3> ? 12 : 4;
    static constexpr std::size_t header_bytes = 36 + value_bytes, key_bytes = 8 + value_bytes;
    static constexpr std::size_t maximum_bytes = header_bytes + 8 + vng::timeline::max_label_bytes +
                                           vng::timeline::max_layer_bytes + vng::timeline::max_keys_per_track * key_bytes;
    static constexpr vng::u64 max_revision = (vng::u64{1} << 53) - 1;
    static_assert(sizeof(vng::f32) == sizeof(vng::u32) && std::numeric_limits<vng::f32>::is_iec559);
    ...
    auto invalid(std::string message) {
        ...
        vng::content::Diagnostic diagnostic;
        diagnostic.code = vng::content::ErrorCode::invalid_document;
        diagnostic.message = std::move(message);
        return std::unexpected(std::move(diagnostic));
    }
    bool valid_value(Value value) {
        const auto scalar = [&](vng::f32 v) { return std::isfinite(v) && v >= minimum && v <= limit; };
        if constexpr (std::same_as<Value, vng::f32>) return scalar(value);
    ...
    vng::content::Result<void> validate(const Snapshot& snapshot, vng::f32 duration = vng::timeline::max_time) {
        ...
        if (track.target != vng::timeline::Target{snapshot.object, std::string(name)})
            return invalid("{Property} edit track must address the same object's {property} property");
        if (!valid_text(track.label, vng::timeline::max_label_bytes) ||
            !valid_text(track.layer, vng::timeline::max_layer_bytes))
        ...
        if (!std::isfinite(duration) || duration < .1F || duration > vng::timeline::max_time)
        ...
        for (const auto& key : track.keys) {
            ...
            if (key.incoming != vng::timeline::Interpolation::hold && key.incoming != vng::timeline::Interpolation::linear)
                return invalid("Unknown {property} key interpolation");
            previous = key.time;
        }
        return {};
    }
    vng::content::Result<void> validate(const Edit& edit) {
    ...
    void integer(std::string& bytes, vng::u64 value, unsigned width) {
        for (unsigned i = 0; i < width; ++i)
            bytes.push_back(static_cast<char>((value >> (i * 8U)) & 255U));
    }
    vng::u64 integer(std::string_view bytes, std::size_t& offset, unsigned width) {
        vng::u64 result{};
        for (unsigned i = 0; i < width; ++i)
            result |= vng::u64(static_cast<unsigned char>(bytes[offset++])) << (i * 8U);
        return result;
    }
    void vector(std::string& bytes, Value value) {
        if constexpr (std::same_as<Value, vng::f32>) integer(bytes, std::bit_cast<vng::u32>(value), 4);
        else for (const auto scalar : {value.x, value.y, value.z})
                integer(bytes, std::bit_cast<vng::u32>(scalar), 4);
    }
    Value vector(std::string_view bytes, std::size_t& offset) {
        if constexpr (std::same_as<Value, vng::f32>)
            return std::bit_cast<vng::f32>(static_cast<vng::u32>(integer(bytes, offset, 4)));
        else {
            vng::Vec3 value;
            for (auto* scalar : {&value.x, &value.y, &value.z})
                *scalar = std::bit_cast<vng::f32>(static_cast<vng::u32>(integer(bytes, offset, 4)));
            return value;
        }
    }

    vng::content::Result<Snapshot> capture(const State& state, vng::u32 object) {
    ...
    vng::content::Result<void> restore(State& state, const Snapshot& snapshot) {
    ...
    vng::content::Result<bool> apply_value(State& state, vng::u32 object, Value value) {
        ...
        if (!valid_value(value)) {
            if constexpr (std::same_as<Value, vng::Vec3>)
        ...
        const vng::timeline::Target target{object, std::string(name)};
        ...
        const auto key = std::ranges::lower_bound(track->keys, state.viewport.time, {}, &vng::timeline::Keyframe::time);
        const auto incoming = key != track->keys.end() && key->time == state.viewport.time
                                  ? key->incoming : vng::timeline::Interpolation::linear;
    ...
    vng::content::Result<Edit> make_edit(vng::u64 base_revision, const State& state, vng::u32 object) {
    ...
    vng::content::Result<void> apply_edit(State& state, const Edit& edit) {
    ...
    vng::content::Result<std::string> encode(const Edit& edit) {
        ...
        if (track) {
            ...
            bytes += track->layer;
            for (const auto& key : track->keys) {
                integer(bytes, std::bit_cast<vng::u32>(key.time), 4);
                vector(bytes, std::get<Value>(key.value));
                integer(bytes, key.incoming == vng::timeline::Interpolation::hold ? 0 : 1, 4);
            }
        }
        return bytes;
    }

    vng::content::Result<Edit> decode(std::string_view bytes) {
        ...
        (edit.*Property).object = static_cast<vng::u32>(integer(bytes, offset, 4));
        (edit.*Property).*Base = vector(bytes, offset);
        const auto count = integer(bytes, offset, 4);
        const auto present = integer(bytes, offset, 4);
        if (present > 1 || count > vng::timeline::max_keys_per_track || (present == 0) != (count == 0))
        ...
        if (present) {
            ...
            if (label_bytes > vng::timeline::max_label_bytes || layer_bytes > vng::timeline::max_layer_bytes ||
            ...
            vng::timeline::Track track{{(edit.*Property).object, std::string(name)},
            ...
            for (vng::u64 i = 0; i < count; ++i) {
                const auto time = std::bit_cast<vng::f32>(static_cast<vng::u32>(integer(bytes, offset, 4)));
                ...
                track.keys.push_back({time, value, incoming == 0 ? vng::timeline::Interpolation::hold : vng::timeline::Interpolation::linear});
            }
